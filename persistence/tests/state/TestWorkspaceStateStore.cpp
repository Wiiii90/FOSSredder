...
TEST(WorkspaceStateStoreTest, SavesAndLoadsTheFullWorkspaceStateRoundTrip) {
  ...
  EXPECT_EQ(loaded.catalog.actors().front()->aliases().at(0).value(),
            "Main Actor");
  EXPECT_EQ(loaded.catalog.actors().front()->aliases().at(1).value(),
            "Primary Actor");
  ASSERT_EQ(loaded.catalog.properties().front()->aliases().size(), 1);
  EXPECT_EQ(loaded.catalog.properties().front()->aliases().at(0).value(),
            "Property Alias");
  ASSERT_EQ(loaded.catalog.contracts().front()->aliases().size(), 1);
  EXPECT_EQ(loaded.catalog.contracts().front()->aliases().at(0).value(),
            "Lease");
  ASSERT_EQ(loaded.catalog.transactions().front()->name(), "Rent");
  ASSERT_EQ(loaded.catalog.transactions().front()->statementId(),
            "statement-1");
  ASSERT_EQ(loaded.catalog.analyses().front()->adjustments().at("actor-1"),
            19.25);
  ASSERT_EQ(loaded.workflow.statementDrafts.front()->name,
            "Imported Statement");
...
TEST(WorkspaceStateStoreTest, ReportsDeletionImpactDeterministically) {
  ...
  EXPECT_EQ(impact.deletedStatementIds,
            std::vector<std::string>({"statement-1"}));
  EXPECT_TRUE(impact.deletedTransactionIds.empty());
  EXPECT_EQ(impact.deletedAnalysisIds,
            std::vector<std::string>({"analysis-1"}));
  EXPECT_EQ(impact.deletedStatementDraftIds,
  ...
  EXPECT_EQ(impact.deletedImportLogIds,
            std::vector<std::string>({"import-log-1"}));
  EXPECT_EQ(impact.deletedExportLogIds,
            std::vector<std::string>({"export-log-1"}));
}
