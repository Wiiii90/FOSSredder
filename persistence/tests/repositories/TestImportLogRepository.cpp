...
TEST(ImportLogRepositoryTest, AddsUpdatesRemovesAndClearsImportLogs) {
  ...
  statementDrafts.addStatementDraft(
      std::make_shared<core::application::importing::draft::StatementDraft>(
          makeStatementDraft()));
  statementDrafts.addStatementDraft(
      std::make_shared<core::application::importing::draft::StatementDraft>(
          makeStatementDraft("statement-draft-2")));

  const auto log = makeImportLog();
  repo.addImportLog(
      std::make_shared<core::application::importing::ImportLog>(log));
  ...
  repo.updateImportLog(
      std::make_shared<core::application::importing::ImportLog>(updated));

  ASSERT_TRUE(repo.getImportLogById("import-log-1").has_value());
  EXPECT_EQ(repo.getImportLogById("import-log-1").value()->status, "running");
  EXPECT_EQ(
      repo.getImportLogById("import-log-1").value()->statementDraftIds,
  ...
  repo.addImportLog(std::make_shared<core::application::importing::ImportLog>(
      makeImportLog("import-log-2")));
