...
TEST(StatementDraftRepositoryTest, AddsUpdatesRemovesAndClearsStatementDrafts) {
  ...
  transactionDrafts.addTransactionDraft(
  ...
  repo.updateStatementDraft(
      std::make_shared<core::application::importing::draft::StatementDraft>(
          draft));

  ASSERT_EQ(repo.getStatementDrafts().size(), 1);
  ASSERT_TRUE(repo.getStatementDraftById("statement-draft-1").has_value());
  EXPECT_EQ(repo.getStatementDraftById("statement-draft-1").value()->name,
            "Imported Statement");
  EXPECT_EQ(
      repo.getStatementDraftById("statement-draft-1").value()->transactionIds,
