...
TEST(AnalysisRepositoryTest, AddsUpdatesRemovesAndClearsAnalyses) {
  ...
  EXPECT_EQ(repo.getAnalysisById("analysis-1").value()->name(),
            "Monthly Analysis");
  EXPECT_EQ(repo.getAnalysisById("analysis-1").value()->type(), "tabular");
  EXPECT_TRUE(repo.getAnalysisById("analysis-1")
                  .value()
                  ->includeCalculationAdjustments());
  EXPECT_EQ(
      repo.getAnalysisById("analysis-1").value()->adjustments().at("actor-1"),
      19.25);
  ...
  EXPECT_FALSE(repo.getAnalysisById("analysis-1")
                   .value()
                   ->includeCalculationAdjustments());
