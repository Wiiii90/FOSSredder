...
TEST(ExportLogRepositoryTest, AddsUpdatesRemovesAndClearsExportLogs) {
  ...
  repo.addExportLog(
      std::make_shared<core::application::exporting::ExportLog>(log));

  ASSERT_EQ(repo.getExportLogs().size(), 1);
  ASSERT_TRUE(repo.getExportLogById("export-log-1").has_value());
  EXPECT_EQ(repo.getExportLogById("export-log-1").value()->targetPath,
            "export.csv");
  ...
  repo.updateExportLog(
      std::make_shared<core::application::exporting::ExportLog>(updated));
  ...
  repo.addExportLog(std::make_shared<core::application::exporting::ExportLog>(
      makeExportLog("export-log-2")));
