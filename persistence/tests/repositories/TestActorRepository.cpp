...
TEST(ActorRepositoryTest, AddsUpdatesRemovesAndClearsActors) {
  ...
  EXPECT_EQ(repo.getActorById("actor-1").value()->aliases().at(0).value(),
            "Main Actor");
  EXPECT_EQ(repo.getActorById("actor-1").value()->aliases().at(1).value(),
            "Primary Actor");
...
TEST(ActorRepositoryTest, UpsertsMissingActors) {
  ...
  EXPECT_EQ(repo.getActorById("actor-upsert").value()->name(),
            "Upserted Actor");
}
