...
std::vector<std::string> loadStatementDraftIds(sqlite3* db,
                                               const std::string& importLogId) {
  ...
  persistence::StmtGuard stmt(
      db, "SELECT statement_draft_id FROM import_log_statement_drafts WHERE "
          "import_log_id = ? ORDER BY position, statement_draft_id;");
  if (!stmt) {
    return ids;
  }
...
void saveStatementDraftIds(sqlite3* db, const std::string& importLogId,
                           const std::vector<std::string>& statementDraftIds) {
  if (!db || importLogId.empty()) {
    return;
  }

  persistence::StmtGuard deleteStmt(
      db, "DELETE FROM import_log_statement_drafts WHERE import_log_id = ?;");
  if (!deleteStmt) {
    return;
  }
  ...
  persistence::StmtGuard insertStmt(
      db, "INSERT OR IGNORE INTO import_log_statement_drafts (import_log_id, "
          "statement_draft_id, position) VALUES (?, ?, ?);");
  if (!insertStmt) {
    return;
  }
...
SqliteImportLogRepository::SqliteImportLogRepository(
    std::shared_ptr<SqliteDb> db)
...
void SqliteImportLogRepository::addImportLog(
    const std::shared_ptr<core::application::importing::ImportLog>& log) {
  const auto db = pimpl_->db->handle();
  writeImportLog(db, log,
                 "INSERT INTO import_logs (id, time, type, file, status, "
                 "message, draft_attached, draft_id, statement_id) VALUES (?, "
                 "?, ?, ?, ?, ?, ?, ?, ?);");
  if (log) {
...
std::vector<std::shared_ptr<core::application::importing::ImportLog>>
SqliteImportLogRepository::getImportLogs() const {
  std::vector<std::shared_ptr<core::application::importing::ImportLog>> logs;
  const auto db = pimpl_->db->handle();
  persistence::StmtGuard stmt(
      db,
      "SELECT id, time, type, file, status, message, draft_attached, draft_id, "
      "statement_id FROM import_logs ORDER BY time DESC, id DESC;");
  if (!stmt) {
    return logs;
  }
  ...
  while (stmt.step() == SQLITE_ROW) {
    ...
    log->statementDraftIds =
        loadStatementDraftIds(pimpl_->db->handle(), log->id);
...
std::optional<std::shared_ptr<core::application::importing::ImportLog>>
SqliteImportLogRepository::getImportLogById(const std::string& id) const {
  const auto db = pimpl_->db->handle();
  persistence::StmtGuard stmt(
      db, "SELECT id, time, type, file, status, message, draft_attached, "
          "draft_id, statement_id FROM import_logs WHERE id = ? LIMIT 1;");
  if (!stmt) {
    return std::nullopt;
  }
...
void SqliteImportLogRepository::removeImportLog(const std::string& id) {
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "DELETE FROM import_logs WHERE id = ?;");
  if (!stmt) {
    return;
  }
  stmt.bindText(1, id);
...
void SqliteImportLogRepository::updateImportLog(
    const std::shared_ptr<core::application::importing::ImportLog>& log) {
  writeImportLog(pimpl_->db->handle(), log,
                 "UPDATE import_logs SET time = ?2, type = ?3, file = ?4, "
                 "status = ?5, message = ?6, draft_attached = ?7, draft_id = "
                 "?8, statement_id = ?9 WHERE id = ?1;");
  if (log) {
    saveStatementDraftIds(pimpl_->db->handle(), log->id,
                          log->statementDraftIds);
  }
}

void SqliteImportLogRepository::upsertImportLog(
    const std::shared_ptr<core::application::importing::ImportLog>& log) {
  writeImportLog(pimpl_->db->handle(), log,
                 "INSERT OR REPLACE INTO import_logs (id, time, type, file, "
                 "status, message, draft_attached, draft_id, statement_id) "
                 "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);");
  if (log) {
    saveStatementDraftIds(pimpl_->db->handle(), log->id,
                          log->statementDraftIds);
