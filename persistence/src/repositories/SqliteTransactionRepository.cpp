...
SqliteTransactionRepository::SqliteTransactionRepository(
...
constexpr auto kSelectTx =
    ...
    " status, actor_id, allocatable, contract_id, valuta, created_at, "
...
void loadTxProperties(sqlite3* db, Transaction& t) {
  persistence::StmtGuard s(db, "SELECT property_id FROM transaction_properties "
                               "WHERE transaction_id = ?;");
  if (!s)
    return;
  s.bindText(1, t.id());
  while (s.step() == SQLITE_ROW)
    t.addPropertyId(s.columnText(0));
}
...
void insertTxProperties(sqlite3* db, const Transaction& t) {
  persistence::StmtGuard s(db, "INSERT OR IGNORE INTO transaction_properties "
                               "(transaction_id, property_id) VALUES (?, ?);");
  if (!s)
    return;
...
void deleteTxProperties(sqlite3* db, const std::string& txId) {
  persistence::StmtGuard s(
      db, "DELETE FROM transaction_properties WHERE transaction_id = ?;");
  if (!s)
    return;
  s.bindText(1, txId);
  s.step();
...
void SqliteTransactionRepository::addTransaction(
  ...
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              ...
                              "  status, actor_id, allocatable, contract_id, "
                              "valuta, created_at, updated_at)"
                              " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
  if (!stmt)
    return;
...
SqliteTransactionRepository::getTransactions() const {
...
SqliteTransactionRepository::getTransactionById(const std::string& id) const {
  if (id.empty())
    return std::nullopt;
  persistence::StmtGuard stmt(
      pimpl_->db->handle(), std::string(kSelectTx) + " WHERE id = ? LIMIT 1;");
  if (!stmt)
    return std::nullopt;
  stmt.bindText(1, id);
...
void SqliteTransactionRepository::updateTransaction(
    const std::shared_ptr<Transaction>& transaction) {
  if (!transaction || transaction->id().empty())
    return;
  persistence::StmtGuard stmt(
      ...
      " statement_id=?, status=?, actor_id=?, allocatable=?, contract_id=?, "
      "valuta=?, created_at=?, updated_at=?"
      " WHERE id=?;");
  if (!stmt)
    return;
...
void SqliteTransactionRepository::upsertTransaction(
    const std::shared_ptr<Transaction>& transaction) {
  if (!transaction || transaction->id().empty())
    return;
  ...
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "INSERT INTO transactions (id, name, booking_date, amount, statement_id, "
      "status, actor_id, allocatable, contract_id, valuta, created_at, "
      "updated_at) "
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
      "ON CONFLICT(id) DO UPDATE SET "
      "name = excluded.name, booking_date = excluded.booking_date, amount = "
      "excluded.amount, "
      "statement_id = excluded.statement_id, status = excluded.status, "
      "actor_id = excluded.actor_id, allocatable = excluded.allocatable, "
...
std::vector<std::shared_ptr<Transaction>>
SqliteTransactionRepository::getTransactionsForContract(
    const std::string& contractId) const {
  std::vector<std::shared_ptr<Transaction>> out;
  persistence::StmtGuard stmt(
      pimpl_->db->handle(), std::string(kSelectTx) + " WHERE contract_id = ?;");
  if (!stmt)
    return out;
