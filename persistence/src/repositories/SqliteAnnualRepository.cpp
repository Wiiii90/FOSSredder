...
std::vector<std::string> loadAnalysisIds(sqlite3* db,
  ...
  persistence::StmtGuard stmt(db, "SELECT analysis_id FROM annual_analyses "
                                  "WHERE annual_id = ? ORDER BY position;");
  if (!stmt) {
    return out;
  }
...
void saveAnalysisIds(sqlite3* db, const std::string& annualId,
  ...
  persistence::StmtGuard deleteStmt(
      db, "DELETE FROM annual_analyses WHERE annual_id = ?;");
  if (!deleteStmt) {
    return;
  }
  ...
  persistence::StmtGuard insertStmt(
      db, "INSERT OR IGNORE INTO annual_analyses (annual_id, analysis_id, "
          "position) VALUES (?, ?, ?);");
  if (!insertStmt) {
    return;
  }
...
std::vector<std::shared_ptr<Annual>>
SqliteAnnualRepository::getAnnuals() const {
...
std::optional<std::shared_ptr<Annual>>
SqliteAnnualRepository::getAnnualById(const std::string& id) const {
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              std::string(kSelectAnnual) +
                                  " WHERE id = ? LIMIT 1;");
  if (!stmt) {
    return std::nullopt;
  }
...
void SqliteAnnualRepository::updateAnnual(
    const std::shared_ptr<Annual>& annual) {
  if (!annual || annual->id().empty()) {
    return;
  }

  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "UPDATE annuals SET name=?, year=?, "
...
void SqliteAnnualRepository::upsertAnnual(
    const std::shared_ptr<Annual>& annual) {
  if (!annual || annual->id().empty()) {
    return;
  }
...
void SqliteAnnualRepository::clearAnnuals() {
  char* err = nullptr;
  sqlite3_exec(pimpl_->db->handle(), "DELETE FROM annuals;", nullptr, nullptr,
               &err);
  if (err) {
    sqlite3_free(err);
  }
