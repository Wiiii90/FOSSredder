...
std::vector<std::string> loadIds(sqlite3* db, const char* sql,
...
void saveIds(sqlite3* db, const std::string& exportLogId,
             const std::vector<std::string>& ids, const char* deleteSql,
...
SqliteExportLogRepository::SqliteExportLogRepository(
    std::shared_ptr<SqliteDb> db)
...
void SqliteExportLogRepository::addExportLog(
    const std::shared_ptr<core::application::exporting::ExportLog>& log) {
  const auto db = pimpl_->db->handle();
  writeExportLog(db, log,
                 "INSERT INTO export_logs (id, time, target_path, status, "
                 "message) VALUES (?, ?, ?, ?, ?);");
  if (log) {
    saveIds(db, log->id, log->annualIds,
            "DELETE FROM export_log_annuals WHERE export_log_id = ?;",
            "INSERT OR IGNORE INTO export_log_annuals (export_log_id, "
            "annual_id, position) VALUES (?, ?, ?);");
    saveIds(db, log->id, log->analysisIds,
            "DELETE FROM export_log_analyses WHERE export_log_id = ?;",
            "INSERT OR IGNORE INTO export_log_analyses (export_log_id, "
            "analysis_id, position) VALUES (?, ?, ?);");
  }
}

std::vector<std::shared_ptr<core::application::exporting::ExportLog>>
SqliteExportLogRepository::getExportLogs() const {
  std::vector<std::shared_ptr<core::application::exporting::ExportLog>> logs;
  const auto db = pimpl_->db->handle();
  persistence::StmtGuard stmt(db,
                              "SELECT id, time, target_path, status, message "
                              "FROM export_logs ORDER BY time DESC, id DESC;");
  if (!stmt) {
    return logs;
  }
  ...
  while (stmt.step() == SQLITE_ROW) {
    ...
    log->annualIds = loadIds(db,
                             "SELECT annual_id FROM export_log_annuals WHERE "
                             "export_log_id = ? ORDER BY position, annual_id;",
                             log->id);
    log->analysisIds =
        loadIds(db,
                "SELECT analysis_id FROM export_log_analyses WHERE "
                "export_log_id = ? ORDER BY position, analysis_id;",
                log->id);
...
std::optional<std::shared_ptr<core::application::exporting::ExportLog>>
SqliteExportLogRepository::getExportLogById(const std::string& id) const {
  const auto db = pimpl_->db->handle();
  persistence::StmtGuard stmt(db,
                              "SELECT id, time, target_path, status, message "
                              "FROM export_logs WHERE id = ? LIMIT 1;");
  if (!stmt) {
    return std::nullopt;
  }
  ...
  log->analysisIds =
      loadIds(db,
              "SELECT analysis_id FROM export_log_analyses WHERE export_log_id "
              "= ? ORDER BY position, analysis_id;",
              log->id);
  return log;
}
...
void SqliteExportLogRepository::updateExportLog(
    const std::shared_ptr<core::application::exporting::ExportLog>& log) {
  const auto db = pimpl_->db->handle();
  writeExportLog(db, log,
                 "UPDATE export_logs SET time = ?2, target_path = ?3, status = "
                 "?4, message = ?5 WHERE id = ?1;");
  if (log) {
    saveIds(db, log->id, log->annualIds,
            "DELETE FROM export_log_annuals WHERE export_log_id = ?;",
            "INSERT OR IGNORE INTO export_log_annuals (export_log_id, "
...
void SqliteExportLogRepository::upsertExportLog(
    const std::shared_ptr<core::application::exporting::ExportLog>& log) {
  const auto db = pimpl_->db->handle();
  writeExportLog(db, log,
                 "INSERT OR REPLACE INTO export_logs (id, time, target_path, "
