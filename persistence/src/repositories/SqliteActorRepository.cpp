...
std::vector<core::domain::Alias>
collectAliases(const core::domain::Actor& actor) {
...
loadActorAliasStats(sqlite3* db, const std::string& actorId) {
...
void loadActorAliases(sqlite3* db, core::domain::Actor& actor) {
  persistence::StmtGuard stmt(
      db, "SELECT alias, hit_count, created_at, updated_at, last_used_at FROM "
          "actor_aliases WHERE actor_id = ? ORDER BY hit_count DESC, "
...
void deleteActorAliases(sqlite3* db, const std::string& actorId) {
  persistence::StmtGuard stmt(db,
                              "DELETE FROM actor_aliases WHERE actor_id = ?;");
  if (!stmt)
    return;
...
void replaceActorAliases(
    sqlite3* db, const core::domain::Actor& actor,
  ...
  persistence::StmtGuard stmt(
      db, "INSERT INTO actor_aliases (actor_id, alias, hit_count, created_at, "
          "updated_at, last_used_at) VALUES (?, ?, ?, COALESCE(NULLIF(?, ''), "
...
void SqliteActorRepository::addActor(const std::shared_ptr<Actor>& actor) {
  ...
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "INSERT OR IGNORE INTO actors (id, name, "
...
std::optional<std::shared_ptr<Actor>>
SqliteActorRepository::getActorById(const std::string& id) const {
  if (id.empty()) {
    return std::nullopt;
  }
...
void SqliteActorRepository::updateActor(const std::shared_ptr<Actor>& actor) {
  ...
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "UPDATE actors SET name = ?, created_at = COALESCE(NULLIF(created_at, "
      "''), ?), updated_at = ? WHERE id = ?;");
  if (!stmt) {
    return;
  }
...
void SqliteActorRepository::clearActors() {
  ...
  sqlite3_exec(pimpl_->db->handle(),
               "DELETE FROM actor_aliases; DELETE FROM actors;", nullptr,
               nullptr, &err);
  if (err) {
    sqlite3_free(err);
  }
