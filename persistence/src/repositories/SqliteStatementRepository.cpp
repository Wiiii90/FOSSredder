...
std::vector<std::string> loadTransactionIds(sqlite3* db,
                                            const std::string& statementId) {
  std::vector<std::string> ids;
  if (!db || statementId.empty())
    return ids;

  persistence::StmtGuard stmt(
      db, "SELECT transaction_id FROM statement_transactions WHERE "
          "statement_id = ? ORDER BY position, transaction_id;");
  if (!stmt)
    return ids;
...
void saveTransactionIds(sqlite3* db, const std::string& statementId,
                        const std::vector<std::string>& transactionIds) {
  if (!db || statementId.empty())
    return;

  persistence::StmtGuard deleteStmt(
      db, "DELETE FROM statement_transactions WHERE statement_id = ?;");
  if (!deleteStmt)
    return;
  ...
  persistence::StmtGuard insertStmt(
      db, "INSERT OR IGNORE INTO statement_transactions (statement_id, "
          "transaction_id, position) VALUES (?, ?, ?);");
  if (!insertStmt)
    return;
...
SqliteStatementRepository::SqliteStatementRepository(
...
void SqliteStatementRepository::addStatement(
    const std::shared_ptr<Statement>& statement) {
  if (!statement || statement->id().empty())
    return;
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "INSERT OR IGNORE INTO statements (id, name, "
                              "created_at, updated_at) VALUES (?, ?, ?, ?);");
  if (!stmt)
    return;
  stmt.bindText(1, statement->id());
  stmt.bindText(2, statement->name());
  stmt.bindText(3, statement->createdAt());
  stmt.bindText(4, statement->updatedAt());
  stmt.step();
  saveTransactionIds(pimpl_->db->handle(), statement->id(),
                     statement->transactionIds());
}

std::vector<std::shared_ptr<Statement>>
SqliteStatementRepository::getStatements() const {
...
SqliteStatementRepository::getStatementById(const std::string& id) const {
  if (id.empty())
    return std::nullopt;
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "SELECT id, name, created_at, updated_at FROM "
                              "statements WHERE id = ? LIMIT 1;");
  if (!stmt)
    return std::nullopt;
  stmt.bindText(1, id);
...
void SqliteStatementRepository::updateStatement(
    const std::shared_ptr<Statement>& statement) {
  if (!statement || statement->id().empty())
    return;
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "UPDATE statements SET name = ?, created_at = ?, "
                              "updated_at = ? WHERE id = ?;");
  if (!stmt)
    return;
  stmt.bindText(1, statement->name());
  stmt.bindText(2, statement->createdAt());
  stmt.bindText(3, statement->updatedAt());
  stmt.bindText(4, statement->id());
  stmt.step();
  saveTransactionIds(pimpl_->db->handle(), statement->id(),
                     statement->transactionIds());
}

void SqliteStatementRepository::upsertStatement(
    const std::shared_ptr<Statement>& statement) {
  if (!statement || statement->id().empty())
    return;
...
void SqliteStatementRepository::clearStatements() {
  char* err = nullptr;
  sqlite3_exec(pimpl_->db->handle(), "DELETE FROM statements;", nullptr,
               nullptr, &err);
  if (err)
    sqlite3_free(err);
}
