...
constexpr auto kSelectProperty = "SELECT id, name FROM properties";
...
loadPropertyAliasStats(sqlite3* db, const std::string& propertyId) {
  ...
  persistence::StmtGuard stmt(
      db, "SELECT alias, hit_count, created_at, updated_at, last_used_at FROM "
          "property_aliases WHERE property_id = ?;");
  if (!stmt)
    return out;
...
void loadPropertyAliases(sqlite3* db, Property& property) {
  persistence::StmtGuard stmt(
      db, "SELECT alias, hit_count, created_at, updated_at, last_used_at FROM "
          "property_aliases WHERE property_id = ? ORDER BY hit_count DESC, "
...
void deletePropertyAliases(sqlite3* db, const std::string& propertyId) {
  persistence::StmtGuard stmt(
      db, "DELETE FROM property_aliases WHERE property_id = ?;");
  if (!stmt)
    return;
...
void replacePropertyAliases(
    sqlite3* db, const Property& property,
  ...
  persistence::StmtGuard stmt(
      db, "INSERT INTO property_aliases (property_id, alias, hit_count, "
...
SqlitePropertyRepository::SqlitePropertyRepository(
...
void SqlitePropertyRepository::addProperty(
    const std::shared_ptr<Property>& property) {
  if (!property || property->id().empty())
    return;
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "INSERT OR IGNORE INTO properties (id, name, "
                              "created_at, updated_at) VALUES (?, ?, ?, ?);");
  if (!stmt)
    return;
  stmt.bindText(1, property->id());
  stmt.bindText(2, property->name());
  stmt.bindText(3, property->createdAt());
  stmt.bindText(4, property->updatedAt());
...
SqlitePropertyRepository::getProperties() const {
...
std::optional<std::shared_ptr<Property>>
SqlitePropertyRepository::getPropertyById(const std::string& id) const {
  if (id.empty())
    return std::nullopt;
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
...
void SqlitePropertyRepository::updateProperty(
    const std::shared_ptr<Property>& property) {
  if (!property || property->id().empty())
    return;
  const auto existing =
      loadPropertyAliasStats(pimpl_->db->handle(), property->id());
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "UPDATE properties SET name = ?, created_at = "
      "COALESCE(NULLIF(created_at, ''), ?), updated_at = ? WHERE id = ?;");
  if (!stmt)
    return;
...
void SqlitePropertyRepository::upsertProperty(
    const std::shared_ptr<Property>& property) {
  if (!property || property->id().empty())
    return;
...
void SqlitePropertyRepository::clearProperties() {
  ...
  sqlite3_exec(pimpl_->db->handle(),
               "DELETE FROM property_aliases; DELETE FROM properties;", nullptr,
               nullptr, &err);
  if (err)
    sqlite3_free(err);
}
