...
void bindNullableText(persistence::StmtGuard& stmt, int column,
...
void bindNullableBlob(persistence::StmtGuard& stmt, int column,
                      const std::vector<uint8_t>& value) {
  if (value.empty()) {
    sqlite3_bind_null(stmt.get(), column);
    return;
  }
  sqlite3_bind_blob(stmt.get(), column, value.data(),
...
int nextTransactionDraftPosition(sqlite3* db) {
  ...
  persistence::StmtGuard stmt(
      db, "SELECT COALESCE(MAX(position), -1) + 1 FROM transaction_drafts;");
  if (!stmt) {
    return 0;
  }
...
void removeTransactionDraftProperties(sqlite3* db, const std::string& id) {
  persistence::StmtGuard stmt(db, "DELETE FROM transaction_draft_properties "
                                  "WHERE transaction_draft_id = ?;");
  if (!stmt) {
    return;
  }
...
void upsertTransactionDraftProperties(
    sqlite3* db,
    const core::application::importing::draft::TransactionDraft& draft) {
  persistence::StmtGuard stmt(
      db, "INSERT OR REPLACE INTO transaction_draft_properties "
          "(transaction_draft_id, property_position, property_id) VALUES (?, "
          "?, ?);");
  if (!stmt) {
    return;
  }

  for (std::size_t propertyIndex = 0; propertyIndex < draft.propertyIds.size();
       ++propertyIndex) {
...
std::shared_ptr<core::application::importing::draft::TransactionDraft>
readDraftRow(sqlite3* db, persistence::StmtGuard& stmt) {
  auto draft =
  ...
  const auto* blob =
      static_cast<const uint8_t*>(sqlite3_column_blob(stmt.get(), 14));
  ...
  draft->status =
      static_cast<core::domain::Transaction::Status>(stmt.columnInt(18));
  draft->createdAt = stmt.columnText(19);
  draft->updatedAt = stmt.columnText(20);

  persistence::StmtGuard properties(
      db, "SELECT property_id FROM transaction_draft_properties WHERE "
          "transaction_draft_id = ? ORDER BY property_position;");
  if (properties) {
...
struct SqliteTransactionDraftRepository::Impl {
  explicit Impl(std::shared_ptr<SqliteDb> db) : db(std::move(db)) {}

  std::shared_ptr<SqliteDb> db;
};

SqliteTransactionDraftRepository::SqliteTransactionDraftRepository(
...
void SqliteTransactionDraftRepository::addTransactionDraft(
    const std::shared_ptr<
        core::application::importing::draft::TransactionDraft>& draft) {
  const auto db = pimpl_->db->handle();
  writeTransactionDraft(
      db, draft,
      "INSERT INTO transaction_drafts (id, statement_draft_id, position, name, "
      "booking_date, valuta, amount, actor_text, property_text, actor_id, "
      "actor_selected, contract_id, contract_selected, metadata, "
      "proof_image_data, type, allocatable, allocatable_selected, status, "
...
std::vector<
    std::shared_ptr<core::application::importing::draft::TransactionDraft>>
SqliteTransactionDraftRepository::getTransactionDrafts() const {
  std::vector<
      std::shared_ptr<core::application::importing::draft::TransactionDraft>>
      drafts;
  const auto db = pimpl_->db->handle();
  ...
  persistence::StmtGuard stmt(
      db,
      "SELECT id, statement_draft_id, position, name, booking_date, valuta, "
      "amount, actor_text, property_text, actor_id, actor_selected, "
      "contract_id, contract_selected, metadata, proof_image_data, type, "
      "allocatable, allocatable_selected, status, created_at, updated_at FROM "
      "transaction_drafts ORDER BY statement_draft_id, position, id;");
  if (!stmt) {
    return drafts;
  }
...
std::optional<
    std::shared_ptr<core::application::importing::draft::TransactionDraft>>
SqliteTransactionDraftRepository::getTransactionDraftById(
  ...
  persistence::StmtGuard stmt(
      ...
      "contract_id, contract_selected, metadata, proof_image_data, type, "
      "allocatable, allocatable_selected, status, created_at, updated_at FROM "
      "transaction_drafts WHERE id = ? LIMIT 1;");
  if (!stmt) {
    return std::nullopt;
  }
...
void SqliteTransactionDraftRepository::removeTransactionDraft(
    const std::string& id) {
  const auto db = pimpl_->db->handle();
...
void SqliteTransactionDraftRepository::updateTransactionDraft(
    const std::shared_ptr<
        core::application::importing::draft::TransactionDraft>& draft) {
  const auto db = pimpl_->db->handle();
  writeTransactionDraft(
      db, draft,
      "UPDATE transaction_drafts SET statement_draft_id = ?2, position = ?3, "
      "name = ?4, booking_date = ?5, valuta = ?6, amount = ?7, actor_text = "
      "?8, property_text = ?9, actor_id = ?10, actor_selected = ?11, "
      "contract_id = ?12, contract_selected = ?13, metadata = ?14, "
      "proof_image_data = ?15, type = ?16, allocatable = ?17, "
      "allocatable_selected = ?18, status = ?19, created_at = ?20, updated_at "
      "= ?21 WHERE id = ?1;",
      false);
}

void SqliteTransactionDraftRepository::upsertTransactionDraft(
    const std::shared_ptr<
        core::application::importing::draft::TransactionDraft>& draft) {
  const auto db = pimpl_->db->handle();
  writeTransactionDraft(
      db, draft,
      "INSERT OR REPLACE INTO transaction_drafts (id, statement_draft_id, "
      "position, name, booking_date, valuta, amount, actor_text, "
      "property_text, actor_id, actor_selected, contract_id, "
      "contract_selected, metadata, proof_image_data, type, allocatable, "
      "allocatable_selected, status, created_at, updated_at) VALUES (?, ?, ?, "
      "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
      true);
}
...
void SqliteTransactionDraftRepository::clearTransactionDrafts() {
  ...
  persistence::StmtGuard deleteProperties(
      db, "DELETE FROM transaction_draft_properties;");
  if (deleteProperties) {
