...
loadAdjustments(sqlite3* db, const std::string& analysisId) {
  ...
  persistence::StmtGuard stmt(
      db, "SELECT adjustment_key, adjustment_value FROM analysis_adjustments "
          "WHERE analysis_id = ? ORDER BY position, adjustment_key;");
  if (!stmt) {
    return out;
  }
...
void saveAdjustments(sqlite3* db, const Analysis& analysis) {
  ...
  persistence::StmtGuard deleteStmt(
      db, "DELETE FROM analysis_adjustments WHERE analysis_id = ?;");
  if (!deleteStmt) {
    return;
  }
  ...
  persistence::StmtGuard insertStmt(
      db, "INSERT OR REPLACE INTO analysis_adjustments (analysis_id, "
          "adjustment_key, adjustment_value, position) VALUES (?, ?, ?, ?);");
  if (!insertStmt) {
    return;
  }
...
std::shared_ptr<Analysis> readAnalysis(persistence::StmtGuard& stmt,
                                       sqlite3* db) {
...
void SqliteAnalysisRepository::addAnalysis(
    const std::shared_ptr<Analysis>& analysis) {
  ...
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "INSERT OR IGNORE INTO analyses (id, name, type, config_json, "
      "filter_spec, export_format, include_calc_adjustments, "
      "export_state_json, snapshot_transactions_json, created_at, updated_at)"
      " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
  if (!stmt) {
    return;
  }
...
std::vector<std::shared_ptr<Analysis>>
SqliteAnalysisRepository::getAnalyses() const {
  std::vector<std::shared_ptr<Analysis>> out;
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "SELECT id, name, type, config_json, filter_spec, export_format, "
      "include_calc_adjustments, export_state_json, "
      "snapshot_transactions_json, created_at, updated_at FROM analyses ORDER "
      "BY id;");
  if (!stmt) {
    return out;
  }
...
std::optional<std::shared_ptr<Analysis>>
SqliteAnalysisRepository::getAnalysisById(const std::string& id) const {
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "SELECT id, name, type, config_json, filter_spec, export_format, "
      "include_calc_adjustments, export_state_json, "
      "snapshot_transactions_json, created_at, updated_at FROM analyses WHERE "
      "id = ? LIMIT 1;");
  if (!stmt) {
    return std::nullopt;
  }
...
void SqliteAnalysisRepository::updateAnalysis(
    const std::shared_ptr<Analysis>& analysis) {
  if (!analysis || analysis->id().empty()) {
    return;
  }
  ...
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "UPDATE analyses SET name=?, type=?, config_json=?, filter_spec=?, "
      "export_format=?, include_calc_adjustments=?, export_state_json=?, "
      "snapshot_transactions_json=?, created_at=?, updated_at=?"
      " WHERE id=?;");
  if (!stmt) {
    return;
  }
...
void SqliteAnalysisRepository::upsertAnalysis(
    const std::shared_ptr<Analysis>& analysis) {
  if (!analysis || analysis->id().empty()) {
    return;
  }
...
void SqliteAnalysisRepository::clearAnalyses() {
  char* err = nullptr;
  sqlite3_exec(pimpl_->db->handle(), "DELETE FROM analyses;", nullptr, nullptr,
               &err);
  if (err) {
    sqlite3_free(err);
  }
