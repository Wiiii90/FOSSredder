...
struct SqliteContractRepository::Impl {
...
core::domain::Alias makeAlias(const std::string& alias, int hitCount,
...
std::unordered_map<std::string, AliasStats>
loadContractAliasStats(sqlite3* db, const std::string& contractId) {
  std::unordered_map<std::string, AliasStats> out;
  persistence::StmtGuard stmt(
      db, "SELECT alias, hit_count, created_at, updated_at, last_used_at FROM "
          "contract_aliases WHERE contract_id = ?;");
  if (!stmt)
    return out;
...
void loadContractAliases(sqlite3* db, Contract& contract) {
  persistence::StmtGuard stmt(
      db, "SELECT alias, hit_count, created_at, updated_at, last_used_at FROM "
          "contract_aliases WHERE contract_id = ? ORDER BY hit_count DESC, "
          "last_used_at DESC, alias ASC;");
  if (!stmt)
    return;
  stmt.bindText(1, contract.id());
  std::vector<core::domain::Alias> aliases;
  while (stmt.step() == SQLITE_ROW) {
    aliases.push_back(makeAlias(stmt.columnText(0), stmt.columnInt(1),
...
void deleteContractAliases(sqlite3* db, const std::string& contractId) {
  persistence::StmtGuard stmt(
      db, "DELETE FROM contract_aliases WHERE contract_id = ?;");
  if (!stmt)
    return;
  stmt.bindText(1, contractId);
  stmt.step();
...
void replaceContractAliases(
    sqlite3* db, const Contract& contract,
  ...
  persistence::StmtGuard stmt(
      db, "INSERT INTO contract_aliases (contract_id, alias, hit_count, "
          "created_at, updated_at, last_used_at) VALUES (?, ?, ?, "
          "COALESCE(NULLIF(?, ''), CURRENT_TIMESTAMP), COALESCE(NULLIF(?, ''), "
          "CURRENT_TIMESTAMP), COALESCE(NULLIF(?, ''), CURRENT_TIMESTAMP));");
  if (!stmt)
    return;

  std::unordered_set<std::string> seen;
  for (const auto& alias : collectAliases(contract)) {
    if (alias.value().empty() || !seen.insert(alias.value()).second)
      continue;
    const auto it = existing.find(alias.value());
    const int hitCount = it == existing.end()
                             ? std::max(1, alias.hitCount())
                             : std::max(1, it->second.hitCount);
    const std::string createdAt =
        it == existing.end() || it->second.createdAt.empty()
            ? alias.createdAt()
            : it->second.createdAt;
    const std::string updatedAt =
        it == existing.end() || it->second.updatedAt.empty()
            ? alias.updatedAt()
            : it->second.updatedAt;
    const std::string lastUsedAt =
        it == existing.end() || it->second.lastUsedAt.empty()
            ? alias.lastUsedAt()
            : it->second.lastUsedAt;
    stmt.reset();
    stmt.bindText(1, contract.id());
...
void insertContractRelations(sqlite3* db, const std::string& contractId,
                             const Contract& c) {
  {
    persistence::StmtGuard s(db, "INSERT OR IGNORE INTO contract_actors "
                                 "(contract_id, actor_id) VALUES (?, ?);");
    if (s)
      for (const auto& id : c.actorIds()) {
        if (id.empty())
          continue;
        s.reset();
        s.bindText(1, contractId);
        s.bindText(2, id);
        s.step();
      }
  }
  {
    persistence::StmtGuard s(db, "INSERT OR IGNORE INTO contract_properties "
                                 "(contract_id, property_id) VALUES (?, ?);");
    if (s)
      for (const auto& id : c.propertyIds()) {
        if (id.empty())
          continue;
        s.reset();
        s.bindText(1, contractId);
...
void deleteContractRelations(sqlite3* db, const std::string& contractId) {
  persistence::StmtGuard a(
      db, "DELETE FROM contract_actors WHERE contract_id = ?;");
  if (a) {
    a.bindText(1, contractId);
    a.step();
  }
  persistence::StmtGuard p(
      db, "DELETE FROM contract_properties WHERE contract_id = ?;");
  if (p) {
    p.bindText(1, contractId);
    p.step();
  }
...
void loadContractRelations(sqlite3* db, Contract& c) {
  {
    persistence::StmtGuard s(
        db, "SELECT actor_id FROM contract_actors WHERE contract_id = ?;");
    if (s) {
      s.bindText(1, c.id());
      while (s.step() == SQLITE_ROW)
        c.addActorId(s.columnText(0));
    }
  }
  {
    persistence::StmtGuard s(
    ...
    if (s) {
      s.bindText(1, c.id());
      while (s.step() == SQLITE_ROW)
        c.addPropertyId(s.columnText(0));
    }
  }
}
...
void SqliteContractRepository::addContract(
    const std::shared_ptr<Contract>& contract) {
  if (!contract || contract->id().empty())
    return;
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "INSERT OR IGNORE INTO contracts (id, name, type, allocatable_mode, "
      "created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?);");
  if (!stmt)
    return;
  stmt.bindText(1, contract->id());
  stmt.bindText(2, contract->name());
...
std::vector<std::shared_ptr<Contract>>
SqliteContractRepository::getContracts() const {
  std::vector<std::shared_ptr<Contract>> out;
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "SELECT id, name, type, allocatable_mode, created_at, updated_at FROM "
      "contracts ORDER BY id;");
  if (!stmt)
    return out;
  while (stmt.step() == SQLITE_ROW) {
...
std::optional<std::shared_ptr<Contract>>
SqliteContractRepository::getContractById(const std::string& id) const {
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "SELECT id, name, type, allocatable_mode, created_at, updated_at FROM "
      "contracts WHERE id = ? LIMIT 1;");
  if (!stmt)
    return std::nullopt;
  stmt.bindText(1, id);
...
void SqliteContractRepository::updateContract(
    const std::shared_ptr<Contract>& contract) {
  if (!contract || contract->id().empty())
    return;
  const auto existing =
      loadContractAliasStats(pimpl_->db->handle(), contract->id());
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
      "UPDATE contracts SET name = ?, type = ?, allocatable_mode = ?, "
      "created_at = COALESCE(NULLIF(created_at, ''), ?), updated_at = ? WHERE "
      "id = ?;");
  if (!stmt)
    return;
...
void SqliteContractRepository::upsertContract(
    const std::shared_ptr<Contract>& contract) {
  if (!contract || contract->id().empty())
    return;
...
void SqliteContractRepository::clearContracts() {
  ...
  sqlite3_exec(pimpl_->db->handle(),
               "DELETE FROM contract_aliases; DELETE FROM contract_actors; "
               "DELETE FROM contract_properties; DELETE FROM contracts;",
               nullptr, nullptr, &err);
  if (err)
    sqlite3_free(err);
}
...
std::vector<std::shared_ptr<Contract>>
SqliteContractRepository::getContractsForActor(
    const std::string& actorId) const {
  std::vector<std::shared_ptr<Contract>> out;
  persistence::StmtGuard ids(
      pimpl_->db->handle(),
...
std::vector<std::shared_ptr<Contract>>
SqliteContractRepository::getContractsForProperty(
...
std::vector<std::string> SqliteContractRepository::getActorIdsForContract(
...
std::vector<std::string> SqliteContractRepository::getPropertyIdsForContract(
    const std::string& contractId) const {
  std::vector<std::string> out;
  persistence::StmtGuard stmt(
      pimpl_->db->handle(),
