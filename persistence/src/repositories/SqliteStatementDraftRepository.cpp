...
loadStatementDraftTransactionIds(sqlite3* db,
                                 const std::string& statementDraftId) {
  ...
  persistence::StmtGuard stmt(
      db, "SELECT transaction_draft_id FROM statement_draft_transactions WHERE "
          "statement_draft_id = ? ORDER BY position, transaction_draft_id;");
  if (!stmt) {
    return ids;
  }
...
void saveStatementDraftTransactionIds(
    sqlite3* db, const std::string& statementDraftId,
  ...
  persistence::StmtGuard deleteStmt(
      db,
      "DELETE FROM statement_draft_transactions WHERE statement_draft_id = ?;");
  if (!deleteStmt) {
    return;
  }
  ...
  persistence::StmtGuard insertStmt(
      db,
      "INSERT OR IGNORE INTO statement_draft_transactions (statement_draft_id, "
      "transaction_draft_id, position) VALUES (?, ?, ?);");
  if (!insertStmt) {
    return;
  }
...
void writeStatementDraft(
    sqlite3* db,
    const std::shared_ptr<core::application::importing::draft::StatementDraft>&
        draft,
    const char* sql) {
  if (!db || !draft) {
    return;
...
SqliteStatementDraftRepository::SqliteStatementDraftRepository(
...
void SqliteStatementDraftRepository::addStatementDraft(
    const std::shared_ptr<core::application::importing::draft::StatementDraft>&
        draft) {
  const auto db = pimpl_->db->handle();
  writeStatementDraft(db, draft,
                      "INSERT INTO statement_drafts (id, name, created_at, "
                      "updated_at) VALUES (?, ?, ?, ?);");
  if (draft) {
...
SqliteStatementDraftRepository::getStatementDrafts() const {
  ...
  while (stmt.step() == SQLITE_ROW) {
    ...
    draft->transactionIds =
        loadStatementDraftTransactionIds(pimpl_->db->handle(), draft->id);
...
std::optional<
    std::shared_ptr<core::application::importing::draft::StatementDraft>>
SqliteStatementDraftRepository::getStatementDraftById(
    const std::string& id) const {
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "SELECT id, name, created_at, updated_at FROM "
                              "statement_drafts WHERE id = ? LIMIT 1;");
  if (!stmt) {
    return std::nullopt;
  }
...
void SqliteStatementDraftRepository::removeStatementDraft(
    const std::string& id) {
  persistence::StmtGuard stmt(pimpl_->db->handle(),
                              "DELETE FROM statement_drafts WHERE id = ?;");
  if (!stmt) {
    return;
  }
  stmt.bindText(1, id);
...
void SqliteStatementDraftRepository::updateStatementDraft(
    const std::shared_ptr<core::application::importing::draft::StatementDraft>&
        draft) {
  const auto db = pimpl_->db->handle();
  writeStatementDraft(db, draft,
                      "UPDATE statement_drafts SET name = ?2, created_at = ?3, "
...
void SqliteStatementDraftRepository::upsertStatementDraft(
    const std::shared_ptr<core::application::importing::draft::StatementDraft>&
        draft) {
  const auto db = pimpl_->db->handle();
  writeStatementDraft(db, draft,
                      "INSERT OR REPLACE INTO statement_drafts (id, name, "
