...
SnapshotIds loadExistingIds(const core::storage::RepositoryBundle& repos) {
  ...
  snapshot.actors = collectRepositoryIds(repos.actors, [](const auto& repo) {
    return repo.getActors();
  });
  snapshot.properties =
      collectRepositoryIds(repos.properties, [](const auto& repo) {
        return repo.getProperties();
      });
  snapshot.contracts =
      collectRepositoryIds(repos.contracts, [](const auto& repo) {
        return repo.getContracts();
      });
  snapshot.statements =
      collectRepositoryIds(repos.statements, [](const auto& repo) {
        return repo.getStatements();
      });
  ...
  snapshot.analyses =
      collectRepositoryIds(repos.analyses, [](const auto& repo) {
        return repo.getAnalyses();
      });
  snapshot.annuals = collectRepositoryIds(repos.annuals, [](const auto& repo) {
    return repo.getAnnuals();
  });
  ...
  snapshot.importLogs =
      collectRepositoryIds(repos.importLogs, [](const auto& repo) {
        return repo.getImportLogs();
      });
  snapshot.exportLogs =
      collectRepositoryIds(repos.exportLogs, [](const auto& repo) {
        return repo.getExportLogs();
      });
...
void syncStatementTransactions(
  ...
  persistence::StmtGuard deleteStmt(db, "DELETE FROM statement_transactions;");
  if (!deleteStmt) {
    return;
  }
  deleteStmt.step();
  ...
  persistence::StmtGuard insertStmt(
      db, "INSERT OR REPLACE INTO statement_transactions (statement_id, "
