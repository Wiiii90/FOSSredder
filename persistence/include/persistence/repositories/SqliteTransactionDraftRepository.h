/**
 * @file
 * persistence/include/persistence/repositories/SqliteTransactionDraftRepository.h
...
class SqliteTransactionDraftRepository
    : public core::ports::repositories::ITransactionDraftRepository {
public:
  /**
   * @brief Create a transaction draft repository for the database at the given
   * path.
  ...
  /**
   * @brief Create a transaction draft repository using an existing database
   * handle wrapper.
  ...
  getTransactionDrafts() const override;
  ...
  std::optional<
      std::shared_ptr<core::application::importing::draft::TransactionDraft>>
  getTransactionDraftById(const std::string& id) const override;
