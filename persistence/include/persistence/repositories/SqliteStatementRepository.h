/**
 * @file
 * persistence/include/persistence/repositories/SqliteStatementRepository.h
...
class SqliteStatementRepository
    : public core::ports::repositories::IStatementRepository {
public:
  /**
  ...
  /**
   * @brief Create a statement repository using an existing database handle
   * wrapper.
  ...
  void addStatement(
      const std::shared_ptr<core::domain::Statement>& statement) override;
  std::vector<std::shared_ptr<core::domain::Statement>>
  getStatements() const override;
  std::optional<std::shared_ptr<core::domain::Statement>>
  getStatementById(const std::string& id) const override;
