 ...
 * @brief Implements main window service wiring between widgets, QML and UI
 * services.
 */
...
MainWindowServices installMainWindowContext(QQmlEngine* qmlEngine,
  ...
  services.navigation = new ui::Navigation(parent);
  services.workspaceStore = new ui::WorkspaceStore(parent);
  services.workspaceSelectors =
      new ui::WorkspaceSelectors(*services.workspaceStore, parent);
  services.workspaceSelection = new ui::WorkspaceSelection(
      *services.workspaceStore, *services.workspaceSelectors, parent);
  services.workspaceCommands =
      new ui::WorkspaceCommands(*services.workspaceStore, parent);
  services.fileSystemBrowser = new ui::FileSystemBrowser(parent);
  auto* application = qobject_cast<QApplication*>(QCoreApplication::instance());
  services.languageService =
      new ui::LanguageService(application, qmlEngine, parent);
  services.settings = new ui::Settings(parent);
  services.status = new ui::Status(parent);
  ...
  QObject::connect(
      ...
      [&window, actions, status = services.status](const QString& filter) {
        ...
        if (!files.isEmpty()) {
          ...
          if (status)
            status->setText(ui::text::mainWindow::selectedStatusPattern().arg(
                files.front()));
        }
      });
