/**
 * @file ui/tests/unit/workspace/TestWorkspaceFacade.cpp
...
void bindWorkspaceRefresh(ui::tests::support::InMemoryWorkspace &workspace,
                          ui::WorkspaceFacade &facade) {
  workspace.setSnapshotChangedCallback(
      [&facade](const core::ports::workspace::WorkspaceSnapshot &snapshot) {
        facade.loadFromState(snapshot);
      });
}
...
TEST(WorkspaceFacadeTest, WSP_FACADE_001_LoadsWorkspaceProjectionAndSelectionIds) {
...
TEST(WorkspaceFacadeTest,
     WSP_FACADE_002_RoutesMutationsThroughCoreBoundaryAndRefreshesUiRows) {
  tests::support::InMemoryWorkspace workspace;
  WorkspaceFacade facade(&workspace, &workspace);
  bindWorkspaceRefresh(workspace, facade);
  auto &models = facade.cache()->models();
...
TEST(WorkspaceFacadeTest, WSP_FACADE_003_PersistsActorAliasesAcrossSelectionChanges) {
  tests::support::InMemoryWorkspace workspace;
  WorkspaceFacade facade(&workspace, &workspace);
  bindWorkspaceRefresh(workspace, facade);
...
TEST(WorkspaceFacadeTest, WSP_FACADE_004_PersistsPropertyAliasesAcrossSelectionChanges) {
  tests::support::InMemoryWorkspace workspace;
  WorkspaceFacade facade(&workspace, &workspace);
  bindWorkspaceRefresh(workspace, facade);
...
TEST(WorkspaceFacadeTest, WSP_FACADE_005_PersistsContractAliasesAcrossSelectionChanges) {
  tests::support::InMemoryWorkspace workspace;
  WorkspaceFacade facade(&workspace, &workspace);
  bindWorkspaceRefresh(workspace, facade);
