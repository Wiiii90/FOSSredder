/**
 * @file ui/tests/unit/workspace/TestWorkspaceCacheModels.cpp
...
TEST(WorkspaceCacheModelsTest,
     WSP_CACHE_MODELS_001_ProjectsAllCollectionsAndRolesFromWorkspaceCatalog) {
  ...
  EXPECT_EQ(models.statements()
                .data(statementIndex, StatementListModel::IdRole)
                .toString(),
            QStringLiteral("statement-1"));
  EXPECT_EQ(models.statements()
                .data(statementIndex, StatementListModel::NameRole)
                .toString(),
            QStringLiteral("January Statement"));

  const QModelIndex txIndex = models.transactions().index(0, 0);
  EXPECT_EQ(
      models.transactions().data(txIndex, TransactionListModel::IdRole).toString(),
      QStringLiteral("tx-1"));
  EXPECT_EQ(models.transactions()
                .data(txIndex, TransactionListModel::StatementIdRole)
                .toString(),
            QStringLiteral("statement-1"));
  EXPECT_EQ(
      models.transactions().data(txIndex, TransactionListModel::NameRole).toString(),
      QStringLiteral("Rent"));
  EXPECT_EQ(models.transactions()
                .data(txIndex, TransactionListModel::BookingDateRole)
                .toString(),
            QStringLiteral("2026-01-05"));
  EXPECT_EQ(models.transactions()
                .data(txIndex, TransactionListModel::ValutaRole)
                .toString(),
            QStringLiteral("EUR"));
  EXPECT_DOUBLE_EQ(models.transactions()
                       .data(txIndex, TransactionListModel::AmountRole)
                       .toDouble(),
                   1250.0);
  EXPECT_EQ(
      models.transactions().data(txIndex, TransactionListModel::StatusRole).toInt(),
      2);
  EXPECT_EQ(models.transactions()
                .data(txIndex, TransactionListModel::ActorIdRole)
                .toString(),
            QStringLiteral("actor-1"));
  EXPECT_EQ(
      models.transactions().data(txIndex, TransactionListModel::TypeRole).toString(),
      QStringLiteral("lease"));
  EXPECT_TRUE(models.transactions()
                  .data(txIndex, TransactionListModel::AllocatableRole)
                  .toBool());
  EXPECT_EQ(models.transactions()
                .data(txIndex, TransactionListModel::PropertyIdsRole)
                .toList()
                .first()
                .toString(),
            QStringLiteral("property-1"));
...
TEST(WorkspaceCacheModelsTest, WSP_CACHE_MODELS_002_RefreshesTransactionTypesAfterContractChanges) {
