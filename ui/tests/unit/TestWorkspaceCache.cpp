/**
 * @file ui/tests/unit/TestWorkspaceCache.cpp
...
#include "ui/state/filters/TransactionFilterModel.h"
#include "ui/workspace/TransactionViewModel.h"

namespace ui {

TEST(WorkspaceCacheTest,
     ExposesFilteredTransactionViewsForWorkspaceCollections) {
...
TEST(WorkspaceCacheTest, AppliesDeletionImpactAcrossModelsAndFilters) {
  WorkspaceCache store;
  store.loadFromState(tests::support::makeWorkspaceSnapshot());
...
TEST(WorkspaceCacheTest, UpdatesTransactionPropertyAssignmentsInPlace) {
  WorkspaceCache store;
  store.loadFromState(tests::support::makeWorkspaceSnapshot());
  ...
  EXPECT_EQ(store.models()
                .transactions()
                .data(rowIndex, TransactionViewModel::PropertyIdsRole)
                .toList(),
...
TEST(WorkspaceCacheTest,
     ResolvesAmountForCommitWithParserAndPersistedFallback) {
  WorkspaceCache store;
  store.loadFromState(tests::support::makeWorkspaceSnapshot());
